...
//                    nBestNetworks.append( settings.getOptionalThreadInfo() );
    ...
    public native void CSModelConstruction(String bsFile,String className, String dataFile,int dcount);

    // Main entry point for Banjo
    public static void main( String _args[] ) throws Exception {

        //System.out.println(CONFIG_MAXVALUECOUNT);
        //System.out.println(Runtime.getRuntime().maxMemory());
        //System.out.println(System.getProperty("java.library.path"));
        System.loadLibrary("InferModel");
        Banjo banjoAppWithThreads =  new Banjo();
        
        banjoAppWithThreads.execute( _args );
        String bfile,cname,dfile;
        int count=5000;
        bfile=banjoAppWithThreads.settings.getValidatedProcessParameter(BANJO.SETTING_OUTPUTDIRECTORY)+ File.separator+banjoAppWithThreads.settings.getValidatedProcessParameter(BANJO.SETTING_FILENAMEFORTOPGRAPH)+".txt";
        cname=banjoAppWithThreads.settings.getValidatedProcessParameter(BANJO.SETTING_PROJECT).trim().replaceAll(" ","");
        dfile=banjoAppWithThreads.settings.getValidatedProcessParameter(BANJO.SETTING_INPUTDIRECTORY)+File.separator+ banjoAppWithThreads.settings.getValidatedProcessParameter(BANJO.SETTING_OBSERVATIONSFILE);

        banjoAppWithThreads.CSModelConstruction(bfile.replaceAll("/","\\\\"),cname,dfile.replaceAll("/","\\\\"),count);
    }
}